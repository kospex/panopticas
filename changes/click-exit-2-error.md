...
The `panopticas` CLI command was exiting with error code 2 when run without any arguments (bare command). This was happening even though the command was successfully displaying the help message.

**Note**: This behavior change occurred in Click v8.1.8 and was breaking the build process, making this fix necessary.

## Root Cause

When running the `panopticas` command without any arguments, Click's default behavior for command groups is to display the help message and exit with code 2. This is the standard behavior for CLI tools when no command is provided.
...
To change the behavior so that showing help exits with code 0 (success), the following changes were made to `src/panopticas/cli.py`:
...
### Verification
```bash
$ .venv/bin/python -m panopticas.cli --help
# Shows help message  
...
2. **Add a default command**: Instead of showing help, execute a default command like `assess`
